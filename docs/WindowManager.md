...
content's `ui::` widget tree too. See [`docs/ui.md`](ui.md) for the widget
...
- **Opaque handles, not pointers.** A `wm::WindowHandle` is a plain
  `unsigned int` (`window_manager.h:12`) — an id you pass around, not
  something you dereference. Internally it keys a
  `std::map<WindowHandle, Window>` guarded by a mutex
  (`window_manager.cpp:16-28`).
- **A de facto global singleton.** All window state lives in file-scope
  `static`s in `window_manager.cpp`; `wm::internal::Initialize()`/
  `Cleanup()` are currently no-ops (`window_manager.cpp:117,200`) — nothing
...
| `WindowSetSize(handle, {w, h})` | Sets the window's outer width/height. A window with zero size in both dimensions is skipped entirely by `Draw()` (`window_manager.cpp:134-136`) — nothing is drawn until you size it. |
...
`Draw()` paints, per window (`window_manager.cpp:138-190`): a bordered,
...
internal `ComputeClientRect()` helper (`window_manager.cpp:79-100`) shared
...
- **No resize/move interaction.** `WindowSetPosition`/`WindowSetSize` only
  ...
  (`window_manager.cpp:192-196`), content already tracks position/size
