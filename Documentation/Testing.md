...
vendored as a submodule at `Vendor/doctest`) that drives the widget
...
keep reading a dangling `FakeInput*`. `Tests/TestInputDriven.cpp` wraps
...
- `Tests/TestContainer.cpp` — pure layout/mutation, no input needed:
...
- `Tests/TestInputDriven.cpp` — via `FakeInput`: button click/drag-off-
