...
float Button::IntrinsicWidth() const {
    return MeasureTextEx(Assets::cozette, text.c_str(), Assets::cozette.baseSize, 0).x + 2 * kPaddingX;
}

float Button::IntrinsicHeight() const { return Assets::cozette.baseSize + 2 * kPaddingY; }
...
void Button::Draw() const {
    ...
    if (isBeingClicked) {
        ...
        UI::DrawTextWithShadow(text.c_str(), drawnRect.x + kPaddingX, drawnRect.y + kPaddingY, NEUTRAL_600);
    } else {
    ...
    if (focused) {
        DrawRectangleLines(drawnRect.x - 2, drawnRect.y - 2, drawnRect.width + 4, drawnRect.height + 4, BLUE_500);
    }
}
